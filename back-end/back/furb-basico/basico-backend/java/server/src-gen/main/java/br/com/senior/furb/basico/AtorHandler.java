...
import static br.com.senior.furb.basico.AtorEntity.SECURITY_RESOURCE;
...
import br.com.senior.furb.basico.Ator.PageRequest;
import br.com.senior.furb.basico.Ator.PagedResults;
import br.com.senior.furb.basico.BasicoConstants;
import br.com.senior.furb.basico.CreateAtor;
import br.com.senior.furb.basico.UpdateAtor;
import br.com.senior.furb.basico.DeleteAtor;
import br.com.senior.furb.basico.RetrieveAtor;
import br.com.senior.furb.basico.ListAtor;
...
import br.com.senior.furb.basico.AtorDTOConverter;
import br.com.senior.platform.translationhub.api.TranslationHubApi;

@HandlerImpl(serviceClass = BasicoConstants.class)
@Component("furb.basico.AtorHandler")
public class AtorHandler implements CreateAtor, CreateMergeAtor, UpdateAtor, DeleteAtor, 
		 UpdateMergeAtor, CreateBulkAtor, RetrieveAtor, ListAtor, 
		 ImportAtor, ExportAtor {
	static final Logger logger = LoggerFactory.getLogger(AtorHandler.class);

	@Inject
	AtorCrudService service;
	
	@Inject
	AtorDTOConverter dtoConverter;
	...
	public Ator createAtor(Ator toCreate) {
		try {
			AtorEntity entity = service.createAtor(dtoConverter.toEntity(toCreate, AtorEntity.class));
			return dtoConverter.toDTO(entity, Ator.class);
	...
	@Transactional
	@Override
	public Ator createMergeAtor(Ator toCreate) {
		try {
			AtorEntity entity = service.createAtor(dtoConverter.toEntity(toCreate, AtorEntity.class));
			return dtoConverter.toDTO(entity, Ator.class);
	...
	public Ator updateAtor(Ator toUpdate) {
		try {
			AtorEntity modified = doUpdate(toUpdate);
			return dtoConverter.toDTO(modified, Ator.class);
	...
	@Transactional
	@Override
	public Ator updateMergeAtor(Ator toUpdate) {
		try {
			AtorEntity entity = service.retrieveAtor(new Ator.Id(toUpdate.id));
			dtoConverter.mergeToEntity(toUpdate, entity);
			entity = service.updateAtor(entity);
			return dtoConverter.toDTO(entity, Ator.class);
	...
	private AtorEntity doUpdate(Ator toUpdate) {
		AtorEntity entity = service.getRepository().findOne(java.util.UUID.fromString(toUpdate.id));
		
		if(entity == null)
			throw new ServiceException(ErrorCategory.OBJECT_NOT_FOUND, translationHubApi.getString("br.com.senior.furb.basico.Ator_notFound", "Ator not found with id " + toUpdate.id));
		
		dtoConverter.toEntity(toUpdate, entity);
		return doUpdate(entity);
	}
	
	private AtorEntity doUpdate(AtorEntity entity) {
		return service.updateAtor(entity);
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_DELETE)
	@Override
	public void deleteAtor(Id id) {
		this.service.deleteAtor(id);
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_VIEW)
	@Transactional(readOnly = true)
	@Override
	public Ator retrieveAtor(Id id) {
		AtorEntity entity = service.retrieveAtor(id);
		return dtoConverter.toDTO(entity, Ator.class);
	}
	...
	@Transactional(readOnly = true)
	@Override
	public PagedResults listAtor(PageRequest pageRequest) {
		...
		Page<AtorEntity> pages = this.service.listAtorPageable(skip, top, orderBy, filter);
		List<AtorEntity> entities = pages.getContent();
		List<Ator> dtos = null;
		
		dtos = entities != null ? entities.stream().map(e -> dtoConverter.toDTO(e, Ator.class, pageRequest.displayFields)).collect(Collectors.toList()) : null;
	...
	public CreateBulkAtorOutput createBulkAtor(CreateBulkAtorInput toCreate) {
		if (toCreate != null) {
			List<Ator> entitiesToCreate = toCreate.entities;
			if (entitiesToCreate != null && !entitiesToCreate.isEmpty()) {
				entitiesToCreate.forEach(bean -> {
					bean.validate();
				});
				
				List<AtorEntity> entities = entitiesToCreate.stream().map(bean -> dtoConverter.toEntity(bean, AtorEntity.class))
						.collect(Collectors.toList());
				this.service.createBulk(entities);
			}
		}
		return new CreateBulkAtorOutput();
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_CREATE)
	@Transactional
	@Override
	public ImportAtorOutput importAtor(ImportAtorInput request) {
		...
		ImportAtorOutput output = new ImportAtorOutput(importer.getId());
		return output;
	}
	...
	public ExportAtorOutput exportAtor(ExportAtorInput request) {
		 ...
		 */
		ExportAtorOutput output = new ExportAtorOutput(
				null /* exporter.getId() */);
		return output;
	}
	...
	class StartImportProcess extends Thread {
		...
		StartImportProcess(Importer importer, ImportConfig config) {
			...
			this.importer.addListener(new AtorImporterListener());
			this.config = config;
		...
		public void run() {
			...
			try {
				...
				try (FileReader reader = new FileReader(blob)) {
					...
					importer.setBeanClass(Ator.class.getName());
					importer.setCrudService(AtorHandler.this.service, (config.batchSize > 0));
		...
		private File getHttpBlobFile(URI uri) {
			...
			try (CloseableHttpClient httpClient = HttpClients.createDefault();
				...
				File blob = File.createTempFile("ator", "blob");
