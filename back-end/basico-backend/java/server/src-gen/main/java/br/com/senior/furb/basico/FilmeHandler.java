...
import static br.com.senior.furb.basico.FilmeEntity.SECURITY_RESOURCE;
...
import br.com.senior.furb.basico.Filme.PageRequest;
import br.com.senior.furb.basico.Filme.PagedResults;
import br.com.senior.furb.basico.BasicoConstants;
import br.com.senior.furb.basico.CreateFilme;
import br.com.senior.furb.basico.UpdateFilme;
import br.com.senior.furb.basico.DeleteFilme;
import br.com.senior.furb.basico.RetrieveFilme;
import br.com.senior.furb.basico.ListFilme;
...
@Component("furb.basico.FilmeHandler")
public class FilmeHandler implements CreateFilme, CreateMergeFilme, UpdateFilme, DeleteFilme, 
		 UpdateMergeFilme, CreateBulkFilme, RetrieveFilme, ListFilme, 
		 ImportFilme, ExportFilme {
	static final Logger logger = LoggerFactory.getLogger(FilmeHandler.class);

	@Inject
	FilmeCrudService service;
	
	@Inject
	FilmeDTOConverter dtoConverter;
	...
	public Filme createFilme(Filme toCreate) {
		try {
			FilmeEntity entity = service.createFilme(dtoConverter.toEntity(toCreate, FilmeEntity.class));
			return dtoConverter.toDTO(entity, Filme.class);
	...
	@Transactional
	@Override
	public Filme createMergeFilme(Filme toCreate) {
		try {
			FilmeEntity entity = service.createFilme(dtoConverter.toEntity(toCreate, FilmeEntity.class));
			return dtoConverter.toDTO(entity, Filme.class);
	...
	public Filme updateFilme(Filme toUpdate) {
		try {
			FilmeEntity modified = doUpdate(toUpdate);
			return dtoConverter.toDTO(modified, Filme.class);
	...
	@Transactional
	@Override
	public Filme updateMergeFilme(Filme toUpdate) {
		try {
			FilmeEntity entity = service.retrieveFilme(new Filme.Id(toUpdate.id));
			dtoConverter.mergeToEntity(toUpdate, entity);
			entity = service.updateFilme(entity);
			return dtoConverter.toDTO(entity, Filme.class);
	...
	private FilmeEntity doUpdate(Filme toUpdate) {
		FilmeEntity entity = service.getRepository().findOne(java.util.UUID.fromString(toUpdate.id));
		
		if(entity == null)
			throw new ServiceException(ErrorCategory.OBJECT_NOT_FOUND, translationHubApi.getString("br.com.senior.furb.basico.Filme_notFound", "Filme not found with id " + toUpdate.id));
		
		dtoConverter.toEntity(toUpdate, entity);
		return doUpdate(entity);
	}
	
	private FilmeEntity doUpdate(FilmeEntity entity) {
		return service.updateFilme(entity);
	}
	...
	@Override
	public Filme retrieveFilme(Id id) {
		FilmeEntity entity = service.retrieveFilme(id);
		return dtoConverter.toDTO(entity, Filme.class);
	}
	...
	@Transactional(readOnly = true)
	@Override
	public PagedResults listFilme(PageRequest pageRequest) {
		...
		Page<FilmeEntity> pages = this.service.listFilmePageable(skip, top, orderBy, filter);
		List<FilmeEntity> entities = pages.getContent();
		List<Filme> dtos = null;
		
		dtos = entities != null ? entities.stream().map(e -> dtoConverter.toDTO(e, Filme.class, pageRequest.displayFields)).collect(Collectors.toList()) : null;
	...
	public CreateBulkFilmeOutput createBulkFilme(CreateBulkFilmeInput toCreate) {
		if (toCreate != null) {
			List<Filme> entitiesToCreate = toCreate.entities;
			if (entitiesToCreate != null && !entitiesToCreate.isEmpty()) {
				entitiesToCreate.forEach(bean -> {
					bean.validate();
				});
				
				List<FilmeEntity> entities = entitiesToCreate.stream().map(bean -> dtoConverter.toEntity(bean, FilmeEntity.class))
						.collect(Collectors.toList());
				this.service.createBulk(entities);
			}
		}
		return new CreateBulkFilmeOutput();
	}
	...
	public ImportFilmeOutput importFilme(ImportFilmeInput request) {
		...
		ImportFilmeOutput output = new ImportFilmeOutput(importer.getId());
		return output;
	}
	...
	public ExportFilmeOutput exportFilme(ExportFilmeInput request) {
		 ...
		 */
		ExportFilmeOutput output = new ExportFilmeOutput(
				null /* exporter.getId() */);
		return output;
	}
	...
	class StartImportProcess extends Thread {
		...
		StartImportProcess(Importer importer, ImportConfig config) {
			...
			this.importer.addListener(new FilmeImporterListener());
			this.config = config;
		...
		public void run() {
			...
			try {
				...
				try (FileReader reader = new FileReader(blob)) {
					...
					importer.setBeanClass(Filme.class.getName());
					importer.setCrudService(FilmeHandler.this.service, (config.batchSize > 0));
		...
		private File getHttpBlobFile(URI uri) {
			...
			try (CloseableHttpClient httpClient = HttpClients.createDefault();
				...
				File blob = File.createTempFile("filme", "blob");
