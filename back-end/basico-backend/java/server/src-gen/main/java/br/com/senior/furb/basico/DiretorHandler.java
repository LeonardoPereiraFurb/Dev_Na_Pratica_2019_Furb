...
import static br.com.senior.furb.basico.DiretorEntity.SECURITY_RESOURCE;
...
import br.com.senior.furb.basico.Diretor.PageRequest;
import br.com.senior.furb.basico.Diretor.PagedResults;
import br.com.senior.furb.basico.BasicoConstants;
import br.com.senior.furb.basico.CreateDiretor;
import br.com.senior.furb.basico.UpdateDiretor;
import br.com.senior.furb.basico.DeleteDiretor;
import br.com.senior.furb.basico.RetrieveDiretor;
import br.com.senior.furb.basico.ListDiretor;
...
import br.com.senior.furb.basico.DiretorDTOConverter;
import br.com.senior.platform.translationhub.api.TranslationHubApi;

@HandlerImpl(serviceClass = BasicoConstants.class)
@Component("furb.basico.DiretorHandler")
public class DiretorHandler implements CreateDiretor, CreateMergeDiretor, UpdateDiretor, DeleteDiretor, 
		 UpdateMergeDiretor, CreateBulkDiretor, RetrieveDiretor, ListDiretor, 
		 ImportDiretor, ExportDiretor {
	static final Logger logger = LoggerFactory.getLogger(DiretorHandler.class);

	@Inject
	DiretorCrudService service;
	
	@Inject
	DiretorDTOConverter dtoConverter;
	...
	public Diretor createDiretor(Diretor toCreate) {
		try {
			DiretorEntity entity = service.createDiretor(dtoConverter.toEntity(toCreate, DiretorEntity.class));
			return dtoConverter.toDTO(entity, Diretor.class);
	...
	@Transactional
	@Override
	public Diretor createMergeDiretor(Diretor toCreate) {
		try {
			DiretorEntity entity = service.createDiretor(dtoConverter.toEntity(toCreate, DiretorEntity.class));
			return dtoConverter.toDTO(entity, Diretor.class);
	...
	public Diretor updateDiretor(Diretor toUpdate) {
		try {
			DiretorEntity modified = doUpdate(toUpdate);
			return dtoConverter.toDTO(modified, Diretor.class);
	...
	@Transactional
	@Override
	public Diretor updateMergeDiretor(Diretor toUpdate) {
		try {
			DiretorEntity entity = service.retrieveDiretor(new Diretor.Id(toUpdate.id));
			dtoConverter.mergeToEntity(toUpdate, entity);
			entity = service.updateDiretor(entity);
			return dtoConverter.toDTO(entity, Diretor.class);
	...
	private DiretorEntity doUpdate(Diretor toUpdate) {
		DiretorEntity entity = service.getRepository().findOne(java.util.UUID.fromString(toUpdate.id));
		
		if(entity == null)
			throw new ServiceException(ErrorCategory.OBJECT_NOT_FOUND, translationHubApi.getString("br.com.senior.furb.basico.Diretor_notFound", "Diretor not found with id " + toUpdate.id));
		
		dtoConverter.toEntity(toUpdate, entity);
		return doUpdate(entity);
	}
	
	private DiretorEntity doUpdate(DiretorEntity entity) {
		return service.updateDiretor(entity);
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_DELETE)
	@Override
	public void deleteDiretor(Id id) {
		this.service.deleteDiretor(id);
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_VIEW)
	@Transactional(readOnly = true)
	@Override
	public Diretor retrieveDiretor(Id id) {
		DiretorEntity entity = service.retrieveDiretor(id);
		return dtoConverter.toDTO(entity, Diretor.class);
	}
	...
	@Transactional(readOnly = true)
	@Override
	public PagedResults listDiretor(PageRequest pageRequest) {
		...
		Page<DiretorEntity> pages = this.service.listDiretorPageable(skip, top, orderBy, filter);
		List<DiretorEntity> entities = pages.getContent();
		List<Diretor> dtos = null;
		
		dtos = entities != null ? entities.stream().map(e -> dtoConverter.toDTO(e, Diretor.class, pageRequest.displayFields)).collect(Collectors.toList()) : null;
	...
	public CreateBulkDiretorOutput createBulkDiretor(CreateBulkDiretorInput toCreate) {
		if (toCreate != null) {
			List<Diretor> entitiesToCreate = toCreate.entities;
			if (entitiesToCreate != null && !entitiesToCreate.isEmpty()) {
				entitiesToCreate.forEach(bean -> {
					bean.validate();
				});
				
				List<DiretorEntity> entities = entitiesToCreate.stream().map(bean -> dtoConverter.toEntity(bean, DiretorEntity.class))
						.collect(Collectors.toList());
				this.service.createBulk(entities);
			}
		}
		return new CreateBulkDiretorOutput();
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_CREATE)
	@Transactional
	@Override
	public ImportDiretorOutput importDiretor(ImportDiretorInput request) {
		...
		ImportDiretorOutput output = new ImportDiretorOutput(importer.getId());
		return output;
	}
	...
	@Transactional(readOnly = true)
	@Override
	public ExportDiretorOutput exportDiretor(ExportDiretorInput request) {
		/*
		 ...
		 */
		ExportDiretorOutput output = new ExportDiretorOutput(
				null /* exporter.getId() */);
		return output;
	}
	...
	class StartImportProcess extends Thread {
		...
		StartImportProcess(Importer importer, ImportConfig config) {
			...
			this.importer.addListener(new DiretorImporterListener());
			this.config = config;
		...
		public void run() {
			...
			try {
				...
				try (FileReader reader = new FileReader(blob)) {
					...
					importer.setBeanClass(Diretor.class.getName());
					importer.setCrudService(DiretorHandler.this.service, (config.batchSize > 0));
		...
		private File getHttpBlobFile(URI uri) {
			...
			try (CloseableHttpClient httpClient = HttpClients.createDefault();
				...
				File blob = File.createTempFile("diretor", "blob");
